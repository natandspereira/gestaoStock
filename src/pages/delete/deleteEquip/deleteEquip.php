...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['equipamentos_id'])) {
    try {
        $db = new Database();
        $conn = $db->getConnection();

        $stmt = $conn->prepare("DELETE FROM cadastro_equipamento WHERE equipamentos_id = :id");
        $stmt->bindValue(':id', intval($_POST['equipamentos_id']), PDO::PARAM_INT);
        $stmt->execute();
